...
public class LeCompteEstBonGraph implements Graph<LeCompteEstBonGraph.State> {
	...
	public class State {
		...
		HashSet<VertexAndWeight<State>> neighbors() {
			...
			SmallSet alreadyChosen = plaques(stack);
			for (int i = 0; i < 10; i++) {
				if (i >= ADD || !alreadyChosen.contains(i)) {
					long s = push(stack, i);
					int compte = compte(s);
					if (compte == IMPOSSIBLE) continue;
					double cost = (i >= ADD ? getOperatorCost(i) : 1);
					neighbors.add(new VertexAndWeight<State>(new State(s, compte), cost));
				}
			}
			return neighbors;
		}
		...
		int compte(long stack) {
			...
			for (int i = 1; i <= n; i++) {
				...
				case ADD: {
					...
					int op2 = s.removeLast();
					int op1 = s.removeLast();
					s.addLast(op1 + op2);					
					break;
				}
				case MINUS: {
				...
				case DIV: {					
					...
					int op2 = s.removeLast();
					int op1 = s.removeLast();										
					if (op2 == 0 || op1 % op2 != 0) return IMPOSSIBLE;
					s.addLast(op1 / op2);
					break;
				}
				default:	
					s.addLast(plaques[get(stack, i)]);
				}
			}
			if (s.size() == 1) {
				return s.getLast();
